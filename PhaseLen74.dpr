...
const   version='7.4';  {enable use not doing the Monte Carlo Analysis}
        ...
        ideal_origin={-10000}-2000;    {earliest date considered by the KS method}
        ideal_limit={3000}20000;   {latest date considered by the KS method}
        calend=1;                  {subscript for calib}
        c14=2;
        c14sd=3;
        calib_year_array_earliest=-20000;  {lower bound of calib_year considered}
        maxidealtrial=100000; {to create ideal distribution for model}
        maxtrial=1000000;  {20000}    {number of samples drawn}
        maxinterval=200;   {number of phase intervals considered}
        maxdate=2500;      {must accommodate # of dates and number of intercepts of the dates}
...
var     date, date_std, date_mean, date_index, date_count,null_array:
          date_array;
        ...
        firstdate, lastdate, daterange, mean_date, ndate, median_date,
          ...
          generate_start, generate_length,generate_middle,generate_std,
        ...
        warn_neg, euclid, big_loop, calibrated_dates, save_generated, MonteCarlo: boolean;
...
begin                  {conceptual array with lower bound of origin}
  offset:=v-origin+1;  {offset in real linear array with bounds from 1..?}
end;

function interpolate(pos,fromss,toss,finddate: integer): integer;
var dif,frac: real;
begin
  dif:=calib[pos,fromss]-calib[pos+1,fromss];
  frac:=1.0*(calib[pos,fromss]-finddate)/dif;
...
begin
  if calib_earliest<calib_year_array_earliest then
    calib_origin:=calib_year_array_earliest {because of size limits}
...
var c,df: ansichar; i: integer; lne: ansistring;
    calendar_year, cal_BP, skip, radiocarbon, error: real;
begin
  {if date[ndate]>6000 then df:='2' else df:='1';} df:='3';
  c:=readchoice('Calibration File: [1]UWTen93, [2]IntCal93, [3]IntCal13, [M]arine93','123M',df);
  if c='1' then calibin:='UWTEN93.14C'
  else if c='2' then calibin:='INTCAL93.14C'
  else if c='3' then calibin:='INTCAL13.14C'
  else calibin:='MARINE93.14C';
  ...
  while not(seekeof(dskin)) do begin
    ...
    if calendar_year>10000 {gets rid of 99999.9 for missing data} then dec(calib_entries) else begin
      calib[calib_entries,calend]:=round(calendar_year);
      calib[calib_entries,c14]:=round(radiocarbon);
      calib[calib_entries,c14SD]:=round(error);
    end;
    skip:=skip; {so it doesn't generate a warning}
  end;
  close(dskin);
...
begin
  ...
  begin
    rslt:=interpolate(pos,c14,calend,finddate);
    inc(pos);
  end;
  next_intercept:=rslt;
end;
...
begin
  ...
  for i:=1 to ndate do begin
    ...
    while pos<>calib_entries do begin
      date_mean[nintercept]:=next_intercept(pos,date[i]);
      date_index[nintercept]:=i;
      if date_mean[nintercept]<>maxint then inc(nintercept);
  ...
  sortdate(nintercept,date_mean,null_array);
   cmedian_date:=date_mean[(nintercept+1) div 2];
  if (nintercept mod 2)=0 then
    cmedian_date:=round((1.0*(cmedian_date+date_mean[(nintercept+2) div 2])/2.0));
  cmedian_date:=roundto(cmedian_date,rounddates);
  sum:=0.0;
  for i:=1 to nintercept do sum:=sum+date_mean[i]/date_count[i];
  cmean_date:=roundto(round(sum/ndate),rounddates);
  cfirstdate:={roundto(}date_mean[1]{,rounddates)};
  clastdate:={roundto(}date_mean[nintercept]{,rounddates)};
  cdaterange:=clastdate-cfirstdate;
  cdate_iqr:=midspread(nintercept,date_mean);
end;
...
begin
  if head then begin
    writeln;
    writeln({'Random Seed ',}'#Dates  Earliest  Latest  Range    IQR   Mean  Median  Ihat    K');
  end;
...
begin
  if head then begin
    writeln(dskout);
    writeln(dskout,'#Dates  Earliest  Latest  Range    IQR   Mean  Median  Ihat    K');
  end;
...
procedure output_date_stats(head:boolean);
{var k: real; }
begin
  if head then begin
    writeln(dskout);
...
begin
  if MonteCarlo then
    Case extension of
      ...
      'C': writeln(fle,n_tests*(this_loop-1)+test_no:4,',',
        this_loop:4,',',test_no:5,',',test_array[0,1]:8,',',rslt_span[test_array[0,2]]:8,',',
        rslt_span[test_array[0,3]]:8,',',rslt_span[test_array[0,4]]:8,',',
        rslt_span[test_array[0,5]]:8,',',test_array[0,6]:8,',',
        ndate:5,',',generate_std:5,',',' "N',ndate:3,'/S',generate_std:3,'"');
    ...
    Case extension of
      ...
      'C': begin
             write(fle,n_tests*(this_loop-1)+test_no:4,',',
               this_loop:4,',',test_no:5,',',firstdate,',',lastdate,',',
...
begin
  ...
  begin
    ...
    minspan:=readint('Minimum True Interval Considered',0,datespan,'0');
...
'<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
  ...
  ihat_warning:=0;
  writeln(dskout);
  writeln(dskout,'Test  Middle  dmax    Dist   Range     IQR    Ihat');
...
procedure command_file(var fle: text; extension: ansichar);
begin  {make output file a systat DATA command file to read the data}
  ...
  case extension of
    ...
    'C': writeln(fle,'Sequence, Loop, Test, Mid_Date, dmax, Dist, Span, IQR, Ihat, Ndate, SD, Group');
    'N': write('Sequence Loop Test Mid_Date dmax Dist Span IQR Ihat Ndate SD Group');
  end
  else
  case extension of
    {'S': begin
...
var finish,temp: integer;  dflt: ansichar;
begin
  dflt:='U';
  calibrated_dates:=readchoice('Generate dates from [C]alibrated Radiocarbon or [U]ncalibrated or Interval',
      'CU',dflt)='C';
  ...
  finish:=readint('True Interval End Date',-100000,100000,'');
  {accounts for entering ages BP or dates}
  if finish<generate_start then begin
    temp:=finish;  finish:=generate_start;  generate_start:=temp;
  end;
  generate_length:=finish-generate_start;
  generate_middle:=round((generate_start+finish)/2);
  model_dist_parms;
end;

procedure date_N_SD_parms;
begin
  ndate:=readint('Number of Dates',1,maxint,'');
  generate_std:=readint('Standard Deviation',0{1},1000,'');
end;
...
function get_date(start,length,std: integer; m: ansichar): integer;
var sample: integer;
begin
  if m='R' then begin
    {$R-}
    if calibrated_dates then sample:={findRCdate(start+random(length))}
       calib_year^[start+random(length)-calib_origin+1] {offset removed for time}
    {$R+}
    ...
    {$R-}
    if calibrated_dates then
      sample:={findrcdate(start+round((ubound+tnormal(lbound,ubound))/(ubound-lbound)*length))}
         calib_year^[start+
           round((ubound+tnormal(lbound,ubound))/(ubound-lbound)*length)-
           calib_origin+1] {offset removed for time}
         {$R+}
...
var i,err,start_date,sampledate: integer;
  sum, dif, maxdif: real;
begin
  sum:=0.0;  err:=0;
  ...
  for i:=1 to nidealtrials do begin
    sampledate:=get_date(start_date,pop_span,date_std[random(ndate-1)+1],
      model);
    sum:=sum+sampledate;
  ...
  if ntrialadj>0 then
    for i:=1 to ndate do begin
      {$R-}
      dif:=abs(1.0*i/ndate-1.0*ideal^[offset(date_mean[i],ideal_origin)]/ntrialadj);
      {$R+}
...
begin
  ...
  for i:=1 to ndate do begin
    if replacement {stds assigned with replacement} then
      date[i]:=get_date(start_date,pop_span,date_std[random(ndate-1)+1],model)
    else {stds assigned without replacement}
      date[i]:=get_date(start_date,pop_span,date_std[i],model)
  end;
  sortdate(ndate,date,date_std);
end;
...
begin
  ...
  if euclid then begin
    for i:=1 to ndate do dst:=dst+sqr(1.0*date[i]-date_mean[i]);
    dst:=sqrt(dst)/ndate;
  end
  else begin
    for i:=1 to ndate do dst:=dst+1*abs(date[i]-date_mean[i]);
    dst:=dst/ndate;
  end;
...
procedure generate_dates(save_generated: boolean;
  setno, n_sets: integer; var ihatn: integer; var ihatsum, ihatsumsq: real);
var i: integer;

begin
  filename:='.ADF';
  if save_generated then begin
    writefile('File for Output Dates',dskout,filename);
    write(dskout,ndate,', 2 # Generated Uncalibrated Dates ',generate_start,' to ',
      generate_start+generate_length);
    if calibrated_dates then write(dskout,' Calibrated Interval') else
      write(dskout,' Uncalibrated Interval');
    writeln(dskout,' Model ',model,' (',ubound:5:2,')',' Randseed: ',orig_randseed);
  end;

  {mean:=0.0; first:=maxint;  last:=-maxint;  sumsq:=0.0;}
  for i:=1 to ndate do begin
    date[i]:=get_date(generate_start,generate_length,generate_std,model);
    date_std[i]:=generate_std;
    if save_generated then writeln(dskout,date[i]:6,generate_std:5);
  end;
  ...
  if save_generated then close(dskout);
end;
...
begin
  save_generated:=readbool('Write Generated Dates to a File','T');
  if not save_generated then
    n_sets:=readint('Number of Sets of Dates to Generate in a Batch',1,maxint,'1')
  else n_sets:=1;
  writeln;
  ...
  if n_sets>1 then
    for setno:=1 to n_sets do
      generate_dates(save_generated,setno,n_sets,ihatn,ihatsum,ihatsumsq)
    else
    repeat
      generate_dates(save_generated,setno,n_sets,ihatn,ihatsum,ihatsumsq);
  ...
  if ihatn>1 then writeln(ihatn,' Trials w/ Ihat defined, Ihat mean=',ihatsum:7:2,'  std=',
...
begin
  ...
  outfile:=readchoice('Output Summary File Type [C]SV, [S]ystat, or [N]one','CSN','C');
  case outfile of
    'C':  extension:='.CSV';
    'S':  extension:='.SYC';
    'N':  extension:='CON';
...
begin  { main program block }
  ...
  if mode<>'G' then begin
    ...
    alloc_matrix(ideal,ideal_limit-ideal_origin+1,sizeof({word} integer));
  end;
